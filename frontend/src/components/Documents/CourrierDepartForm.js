...
const CourrierDepartForm = ({ isEdit = false, documentId = null, onClose, onSave }) => {
  const { user } = useAuth();
  ...
  const fetchDocument = async () => {
    try {
      ...
      setFormData({
        date_depart: doc.metadata?.date_depart || new Date().toISOString().split('T')[0],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        description: `Courrier départ - De: ${formData.expediteur} - À: ${formData.destinataire}`,
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">Gestion du courrier sortant</p>
          </div>
        </div>
        {onClose && (
          <button
...
export default CourrierDepartForm;